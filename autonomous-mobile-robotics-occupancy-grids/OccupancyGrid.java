...
public class OccupancyGrid {
  ...
  private int[] visitCounts; // Tracks visit counts for each cell
  ...
  public OccupancyGrid(Supervisor r,
    ...
    this.visitCounts = new int[this.num_row_cells * this.num_col_cells];
  ...
  // Method to get visit count
  public int get_visit_count(int cellIndex) {
      if (cellIndex >= 0 && cellIndex < visitCounts.length) {
          return visitCounts[cellIndex];
      }
      return 0;
  }
  
  public void update_visit_count(Pose robotPose) {
        int cellIndex = get_cell_index(robotPose);
        if (cellIndex >= 0 && cellIndex < visitCounts.length) {
            visitCounts[cellIndex]++;
        }
    }
  ...
  // Helper method to get cell index based on robot's pose
  private int get_cell_index(Pose robotPose) {
    double x = robotPose.getX() + this.arena_width / 2.0;
    double y = this.arena_height / 2.0 - robotPose.getY();
    int col = (int) (x / (this.arena_width / this.num_row_cells));
    int row = (int) (y / (this.arena_height / this.num_col_cells));
    if (col >= 0 && col < this.num_row_cells && row >= 0 && row < this.num_col_cells) {
        return row * this.num_row_cells + col;
    }
    return -1; // Return -1 if the cell index is out of bounds
  }
  ...
  // ---------------------------------------------------------------------------
   private double invSensorModel(Pose p, double x, double y) {
    ...
    double l_occ = Math.log(2000000000 / (0.01));  
    double l_free = Math.log(0.01);
    double r = Math.sqrt((deltaX*deltaX)+(deltaY*deltaY));     
    double phi = Math.atan2(deltaY, deltaX) - p.getTheta();          
    double logodds = lprior;                                         
    ...
    for (int x = 0; x < prox_sensors.get_number_of_sensors(); x++) {
        Pose curr_sensorPose = prox_sensors.get_relative_sensor_pose(x);
        double curr_diff = Math.abs(curr_sensorPose.getDeltaTheta(phi));
    ...
    double zt = prox_sensors.get_value(k);      
    double zmax = prox_sensors.get_maxRange();  
    Pose closest_sensorPose = prox_sensors.get_relative_sensor_pose(k);

    
    if (zmax < zt || Math.abs(closest_sensorPose.getDeltaTheta(phi)) > HALFBETA) {
        return lprior;
    }
   
   
    if (zt < zmax && Math.abs(closest_sensorPose.getDeltaTheta(phi)) < HALFBETA) {
       
       
       if ((zt < zmax) && (Math.abs(r - zt) < HALFALPHA)) {
            return l_occ;
        }
        
        if ((r > Math.min(zmax, zt + HALFALPHA)) ||
            (Math.abs(closest_sensorPose.getDeltaTheta(phi)) > HALFBETA)) {
            return lprior;
        }
